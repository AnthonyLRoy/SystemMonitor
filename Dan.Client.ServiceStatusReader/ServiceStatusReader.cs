// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ServiceStatusReader.cs" company="DanSys">
//   
// </copyright>
// <summary>
//   The service status reader.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace Dan.Client.ServiceStatusReader
{
    ...
    /// </summary>
    [MetricReader(true)]
    public class ServiceStatusReader : IMetricReader
    {
        ...
        /// </summary>
        private List<ServiceStatusTask> _performanceObjects;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ServiceStatusReader"/> class.
        /// </summary>
        public ServiceStatusReader()
        {
            this._schedule = null;
        ...
        /// </summary>
        /// <param name="serviceStatusTask">
        /// The service Status Task.
        /// </param>
        /// <returns>
        /// The <see cref="Task"/>.
        /// </returns>
        private async Task<MetricValue> ReadCounterAsync(ServiceStatusTask serviceStatusTask)
        {
            var data = serviceStatusTask.Data;
            var machineInternal = string.IsNullOrEmpty(data.machinename) ? Environment.MachineName : data.machinename;
            var sc = new ServiceController(data.servicename) { MachineName = machineInternal };
            var result = (int)sc.Status;
            return new MetricValue(
                result, 
                result, 
                string.Format("{0}-{1}", machineInternal, data.servicename), 
                DateTime.Now);
        }
        ...
        /// The <see cref="IEnumerable"/>.
        /// </returns>
        private IEnumerable<ServiceStatusTask> SetUpCounters()
        {
            var serializer = new XmlSerializer(typeof(services));

            using (var stream = File.OpenRead("ServiceStatusCongig.XML"))
            {
                Log.Info("Attempting to access Active Counters");
                return
                    ((services)serializer.Deserialize(stream)).service.Select(
                        servicetask => new ServiceStatusTask(servicetask)).ToList();
            }
