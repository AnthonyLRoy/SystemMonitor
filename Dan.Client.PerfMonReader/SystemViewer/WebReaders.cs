// --------------------------------------------------------------------------------------------------------------------
// <copyright file="WebReaders.cs" company="">
//   
// </copyright>
// <summary>
//   The web readers.
...
namespace SystemViewer
{
    ...
    public static class WebReaders
    {
        ...
        public static void Init()
        {
            ...
            foreach (var instanceReader in
                Readers().Select(reader => Activator.CreateInstance((Type)reader) as IWebMetricsReader<MetricValue>))
            {
        ...
        /// The <see cref="Task"/>.
        /// </returns>
        internal static async Task<List<MetricValue>> GetEventsByGroup(GroupName groupName)
        {
            var metricValues =
                await
                ActiveReaders.First(x => x.Key == groupName.Id).Value.GetMetricsByGroupNameAsync(groupName.Groupname);
            return metricValues;
        }
